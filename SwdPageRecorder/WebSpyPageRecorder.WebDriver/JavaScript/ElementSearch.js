...
(function() {
  var WebSpy_Page_Recorder, addStyle, bye, createCommand, dbg, getInputElementsByTypeAndValue, getPageXY, getCssSelectorOF, getElementId, getPathTo, handler, hello, prev, preventEvent, pseudoGuid, rightClickHandler, say;
...
getElementId = function(element) {
  ...
  createCommand = function(jsonData) {
    ...
    document.WebSpypr_command = myJSONText;
  ...
  document.WebSpy_prevActiveElement = void 0;
  ...
  WebSpy_Page_Recorder = (function() {
    function WebSpy_Page_Recorder() {}

    WebSpy_Page_Recorder.prototype.getMainWinElement = function() {
      return document.getElementById('WebSpyPR_PopUp');
    };

    WebSpy_Page_Recorder.prototype.displayWebSpyForm = function(x, y) {
      var el;
      hello("displayWebSpyForm");
    ...
    WebSpy_Page_Recorder.prototype.closeForm = function() {
      return document.getElementById('WebSpyPR_PopUp').style.display = 'none';
    };

    WebSpy_Page_Recorder.prototype.createElementForm = function() {
      var closeClickHandler, element;
      hello("createElementForm");
      element = document.createElement("div");
      element.id = 'WebSpyPR_PopUp';
      if (document.body != null) {
        document.body.appendChild(element);
      } else {
        say("createElementForm Failed to inject element WebSpyPR_PopUp. The document has no body");
      }
      closeClickHandler = "";
            element.innerHTML = '\
        <table id="WebSpyTable">\
            <tr>\
              <td>Code identifier</td>\
              <td>\
                    <div id="WebSpyPR_PopUp_Element_Name">\
                        <span id="WebSpyPR_PopUp_CodeID">\
                            <input type="text" id="WebSpyPR_PopUp_CodeIDText">\
                        </span>\
                        <span id="WebSpyPR_PopUp_CodeClose"></span>\
                        <span id="WebSpyPR_PopUp_CloseButton" onclick="document.WebSpy_Page_Recorder.closeForm()">X</span>\
                     </div>\
              </td>\
            </tr>\
            <tr>\
              <td>Element</td>\
              <td><span id="WebSpyPR_PopUp_ElementName">Element</span></td>\
            </tr>\
            <tr>\
              <td>Id:</td>\
            ...
            <tr>\
              <td>XPath:</td>\
              <td><span id="WebSpyPR_PopUp_XPathLocator">Element</span></td>\
            </tr>\
            <tr>\
              <td>Css:</td>\
    ...
    return WebSpy_Page_Recorder;

  })();

  addStyle(".highlight { background-color:silver !important}");

  addStyle("table#WebSpyTable {             background-color:white;             border-collapse:collapse;           }                       table#WebSpyTable,table#WebSpyTable th, table#WebSpyTable td  {             font-family: Verdana, Arial;             font-size: 10pt;             padding-left:10pt;             padding-right:10pt;             border-bottom: 1px solid black;            }");

  addStyle("input#WebSpyPR_PopUp_CodeIDText {             display:table-cell;             width:95%;          }");

  addStyle("span#WebSpyPR_PopUp_CloseButton {              display:table-cell;            -moz-border-radius: 4px;            -webkit-border-radius: 4px;            -o-border-radius: 4px;            border-radius: 4px;            border: 1px solid #ccc;            color: white;            background-color: #980000;            cursor: pointer;            font-size: 10pt;            padding: 0px 2px;            font-weight: bold;            position: absolute;            right: 3px;            top: 8px;          }");

  /*
  addStyle "span#WebSpyPR_PopUp_CloseButton {  
              display:table-cell; 
              width:10px; 
  ...
  */
  ...
  addStyle("div#WebSpyPR_PopUp {             display:none;           }           div#WebSpyPR_PopUp_Element_Name {             display:table;             width: 100%;           }");
  ...
  */
  ...
  if (document.body != null) {
    ...
    document.WebSpy_Page_Recorder = new WebSpy_Page_Recorder();
    document.WebSpy_Page_Recorder.createElementForm();
  } else {
    say("Document has no body tag... Injecting empty web");
    document.WebSpy_Page_Recorder = "STUB. Document has no body tag :(";
  }
